...
const MyCards = ({isCard}) => {
    return (
        <div className={`my-cards-window settings-windows ${isCard ? 'active' : false}`}>
            <h3>My Card</h3>
                    ...
                    <div className="cards-overflow">
                        {
                            cards.map(card => (
                                <div className="card" onClick={ () => {
                                    popoutWindow('deleteCard')
                                } }></div>
                            ))
                        }
                    </div>
                </div>
                <div className="carousel-pins">
                    {
                        cards.map(card => (
